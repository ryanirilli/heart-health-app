...
export function TopographyCanvas({ className = "" }: TopographyCanvasProps) {
  ...
  useEffect(() => {
    ...
    const getThemeColors = () => {
      ...
      // Parse HSL values (format: "220 18% 8%")
      const parseBg = bgHsl.split(" ");
      const parseFg = fgHsl.split(" ");

      return {
        bg: `hsl(${parseBg[0]}, ${parseBg[1]}, ${parseBg[2]})`,
        fg: parseFg,
      };
    };
    ...
    const draw = (time: number) => {
      ...
      const cellSize = 10; // Smaller cells = more detail
      const cols = Math.ceil(width / cellSize) + 1;
      const rows = Math.ceil(height / cellSize) + 1;
      const scale = 0.0025; // Slightly smaller scale = more variation
      const timeOffset = timeRef.current;

      // Generate height map with time-based animation
      ...
      // Draw contour lines at different elevations - more levels for denser lines
      ...
      for (let level = 0; level < contourLevels; level++) {
        ...
        // Vary opacity based on elevation - creates depth
        const normalizedLevel = level / contourLevels;
        const opacity =
          baseOpacity +
          normalizedLevel * 0.25 * Math.sin(normalizedLevel * Math.PI);

        // Use theme foreground color with higher lightness for more contrast
